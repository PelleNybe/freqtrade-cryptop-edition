...
class ConsecutiveLossGuard(IProtection):
    ...
    def short_desc(self) -> str:
        return (
            f"{self.name} - Stops trading if {self._trade_limit} consecutive losses occur "
            f"within {self.lookback_period_str}."
        )

    def _reason(self) -> str:
        return (
            f"{self._trade_limit} consecutive losses in {self.lookback_period_str}, "
            f"locking {self.unlock_reason_time_element}."
        )

    def _consecutive_loss_guard(
        ...
        if consecutive_losses >= self._trade_limit:
            self.log_once(
                f"Trading stopped due to {self._trade_limit} consecutive losses "
                f"within {self._lookback_period} minutes.",
                logger.info,
            )
            until = self.calculate_lock_end(loss_trades)
    ...
    def global_stop(self, date_now: datetime, side: LongShort) -> ProtectionReturn | None:
        if self._disable_global_stop:
            return None
        return self._consecutive_loss_guard(date_now, None, side)

    def stop_per_pair(
        self, pair: str, date_now: datetime, side: LongShort
    ) -> ProtectionReturn | None:
        return self._consecutive_loss_guard(date_now, pair, side)
